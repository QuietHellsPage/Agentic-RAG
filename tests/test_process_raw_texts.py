...
from src.helpers.process_raw_texts import Processor
...
class TestProcessRawTexts:
    ...
    def test_init_and_repr(self):
        ...
        assert str(processor_overwrite_true_default) == "'Processor'(self.overwrite=True)"
    ...
    def test_process_corpus(self, tmp_path):
        ...
        with patch.object(processor, 'output_md_collection_path', output_dir):
            with patch('glob.glob', return_value=[str(f) for f in input_dir.glob('*.pdf')]):
    ...
    def test_handles_empty_pdf(self, processor):
        """Verify handling of empty PDF (no markdown generated)"""
